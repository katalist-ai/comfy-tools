...
class SkeletonVAE(L.LightningModule):
    ...
    def __init__(self, input_dim=36, hidden_dim=32, latent_dim=4, learning_rate=0.001, weight_decay=0.001):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(input_dim, hidden_dim), nn.LeakyReLU(0.2), nn.Linear(hidden_dim, hidden_half), nn.LeakyReLU(0.2)
        )
        ...
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, hidden_half),
            nn.LeakyReLU(0.2),
            nn.Linear(hidden_half, hidden_dim),
            nn.LeakyReLU(0.2),
